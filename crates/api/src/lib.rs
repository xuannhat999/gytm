...
impl YClient {
    ...
    // Hàm tạo HeaderMap cho mỗi request POST đến InnerTube
    ...
    pub async fn new(config: AppConfig) -> Result<Self> {
        ...
        let client_version = extract_between(&response_text, "INNERTUBE_CLIENT_VERSION\":\"", "\"")
            .ok_or_else(|| YError::AuthError("No Clien Version".to_string()))?;
    ...
    pub async fn get_lib_data(&self) -> Result<Value> {
        let url = format!(
            "https://music.youtube.com/youtubei/v1/browse?key={}&alt=json",
